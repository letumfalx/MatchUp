...
Object.defineProperties(GMPixi.Math, {
    ...
    shuffle: {
        value: function shuffle(array) {
            ...
            if(GMPixi.checkType(array, String)) {
                ...
                throw TypeError('Unable to shuffle input. Input must be a string or an array to shuffle.');
            }
    ...
    next: {
        value: function(base, increment, min, max) {
            if(!GMPixi.Math.canBeNumber(base)) {
                throw TypeError('Base number cannot convert to number.');
            }
            
            increment = Math.toNumber(increment, 1);
            min = Math.toNumber(min, 0);
            max = Math.toNumber(max, 99);
